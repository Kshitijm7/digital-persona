...
Improve viseme lipsync alignment
No tasks in progress


great work, can you further improve viseme lipsync to align properly with audio, looks bit off and not align with audio, search online research and latest formation and experiments and articles for realistic viseme and create guide in relevent @project_docs folder and then update the codebase to implement those improvements


11 previous messages
Implemented. I added a research-backed lip-sync alignment upgrade and documented it.

What changed

Added audio-clock-based viseme scheduling with output-latency compensation, state-aware hysteresis, and short-window coarticulation in src/lib/lipsync-engine.ts (notable around lines 84, 280, 310, 339, 371).
Switched playback level detection to faster time-domain RMS in src/lib/audio-streamer.ts (around lines 50, 182).
Tuned wawa-lipsync integration/analyzer settings (lower lag, tighter persistence) in src/hooks/useAudioProcessor.ts (around lines 264, 278, 295).
Reduced onset lag in avatar smoothing (fast attack / slower release + raw/smoothed max for lip-sync input) in src/components/canvas/Avatar.tsx (around lines 202, 209, 227).
Added full guide in project_docs/avatar_and_animation/viseme_lipsync_alignment_2026.md and linked it from project_docs/avatar_and_animation/README.md.
Validation

npm.cmd run typecheck passed.
npx.cmd eslint src/lib/lipsync-engine.ts src/hooks/useAudioProcessor.ts src/lib/audio-streamer.ts src/components/canvas/Avatar.tsx passed.
Inference used

Since this pipeline does not receive native per-phoneme timestamps from Gemini audio output, I emulated timestamped viseme playback via audio-clock scheduling (getOutputTimestamp/currentTime + latency compensation), which is the closest real-time equivalent to provider viseme events.
Key sources used
...
1 previous message
Your research summary is strong, but for your current stack (RPM blendshapes + WebAudio + wawa-lipsync) only part of it is directly actionable.

What’s high-value for your system

Audio-clock alignment and output-latency compensation (high confidence, immediately useful).
Coarticulation and anti-jitter viseme transitions (high confidence, already aligned with your pipeline).
Affective decoupling (keep speech mouth mechanics separate from expressive channels) (high confidence).
Perceptual evaluation over pure SyncNet-style lip metrics (high confidence).
What is overkill right now

Rebuilding to NeRF/3DGS/diffusion backends.
Training MoE multilingual models (MuEx-style) in-house.
Full symbolic + neural hybrid research pipelines.
Pragmatic improvements (low complexity, high ROI)

Add runtime tuning controls for lip-sync timing.
Expose CLOCK_COMPENSATION_RATIO, ANTICIPATION_WINDOW_MS, silence hold, and maxVisemeDuration in UI for quick A/B tuning per device.
Code touchpoints: lipsync-engine.ts, useAudioProcessor.ts, ConfigPanel.tsx.
Add adaptive noise-floor gating.
Track short rolling floor during silence and adapt speaking threshold dynamically; this reduces false mouth motion in noisy rooms.
Code touchpoints: useAudioProcessor.ts, lipsync-engine.ts.
Add lightweight transcript lookahead (optional, not heavy ML).
Use partial transcript characters/phoneme heuristics to pre-bias next viseme 30–70ms ahead (fallback to audio-only when unavailable).
Code touchpoints: useSessionManager.ts, viseme-map.ts, lipsync-engine.ts.
Improve affect while speaking without mouth conflicts.
Keep upper-face micro-expression active (brows/eyes), but continue suppressing conflicting smile/frown mouth channels during speech.
Code touchpoint: emotion-engine.ts.
Add a small perceptual QA harness.
Track onset lag (ms), silence drift, viseme switch rate, and short human MOS (1–5). This gives practical THEval-style quality signals without full benchmark complexity.
Suggested place: scripts/ + project_docs/.
You’re already on the right path: your current code has clock compensation, anticipation blending, and faster onset handling. Next best move is making those parameters tunable and adding adaptive gating + lightweight evaluation.
...
https://docs.readyplayer.me/ready-player-me/api-reference/avatars/morph-targets/apple-arkit
https://cvpr.thecvf.com/virtual/2025/poster/32963
https://wacv.thecvf.com/virtual/2026/poster/445
https://papers.cool/arxiv/2511.04520
https://papers.cool/arxiv/2510.06612
...
3 previous messages
Implemented the high-value upgrades (without heavy model complexity):

Added runtime lip-sync tuning state.
New tuning model + defaults + patch/reset actions in useLipSyncStore.ts.
Includes timing controls, coarticulation window, switch hysteresis, and adaptive noise-floor params.
Added adaptive noise-floor gating in lip-sync engine.
updateFromAudioLevel now takes tuning and uses dynamic floor estimation before viseme gain mapping in lipsync-engine.ts.
Added getEffectiveFloor for ambient-noise adaptation in lipsync-engine.ts.
Clock compensation, viseme switch intervals, anticipation window, and silence hold now use runtime tuning values in lipsync-engine.ts.
Wired tuning into the avatar runtime path.
Avatar now passes both wawaLipsync and live tuning to the engine in Avatar.tsx.
Exposed a new “Lip Sync Tuning” panel in UI.
Added controls for adaptive floor, clock ratio/clamp, anticipation window, silence hold, threshold offset, adapt/release lambdas, plus reset in ConfigPanel.tsx.
Updated docs.
Added the runtime tuning section in viseme_lipsync_alignment_2026.md.
Validation:

npm.cmd run typecheck passed.
npx.cmd eslint src/store/useLipSyncStore.ts src/lib/lipsync-engine.ts src/components/canvas/Avatar.tsx src/components/chat/ConfigPanel.tsx passed.
If you want, next I can add 3 quick presets (Low Latency, Balanced, Noisy Room) as one-click buttons in the same panel.


add presets also add these
## Tuning Guide
...
to config as well
add any other important that needs to configure to the config and to the camera.json for persistance. 



5 previous messages
Implemented. Presets, guide text, and persistent advanced lip-sync tuning are now in place.

Added full tuning schema + presets (Low Latency, Balanced, Noisy Room) in useLipSyncStore.ts.
Added/connected new runtime knobs in lip-sync engine: anticipation weight, state switch intervals, state lambdas, adaptive noise-floor bounds in lipsync-engine.ts.
Applied analyserSmoothing and visemePersistenceMs live to playback/wawa pipeline in useAudioProcessor.ts.
Added a full “Lip Sync Tuning” section with preset buttons, all key controls, and your tuning guide directly in the UI in ConfigPanel.tsx.
Synced tuning into scene config model and bootstrapped from saved config in SceneConfigContext.tsx.
Persisted lipSyncTuning through API merge in route.ts.
Added persisted defaults in camera.json.
Updated docs to mention presets + persistence in viseme_lipsync_alignment_2026.md.
Validation:

npm.cmd run typecheck passed.
ESLint on touched TS/TSX files passed.
Note:

Changes apply live immediately in UI.
Persistence to camera.json still happens via the existing Save to JSON button.
...
2 previous messages
Applied a fix for the lip-sync regression and hardened the pipeline.

What I changed
Added a robust fallback when wawa-lipsync keeps returning viseme_sil despite audible speech in lipsync-engine.ts.
Prevented silent-state lockups by normalizing active state when non-sil viseme is scheduled in lipsync-engine.ts.
Restored analyser dB range to values compatible with wawa-lipsync in useAudioProcessor.ts.
Added presets + full tuning controls + your tuning guide in ConfigPanel.tsx.
Added persistent lipSyncTuning support end-to-end:
schema/bootstrap in SceneConfigContext.tsx
API merge in route.ts
saved defaults in camera.json
Validation
npm.cmd run typecheck passed.
ESLint on touched TS/TSX files passed.
What to do now
Reload the app once.
In Config → Lip Sync Tuning, click Balanced preset.
Ensure Features → Lip Sync is enabled.
Test speech, then click Save to JSON (persists to camera.json).
If it still looks off, switch preset to Low Latency and test again.
